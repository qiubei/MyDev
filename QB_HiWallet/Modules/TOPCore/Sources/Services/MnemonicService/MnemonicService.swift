...
public class MnemonicService: MnemonicServiceInterface {
    ...
    //增加主私钥生成
    public func masterKey(from mnemonic:String) ->String {
        let seed = self.seed(from: mnemonic)
        return Crypto.HMACSHA512(key:"Bitcoin seed".data(using: .nonLossyASCII)!, data: seed.data(using: .nonLossyASCII)!).toHexString()
    }
