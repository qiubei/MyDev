...
public class LocalTxModel: Object {
    ...
    @objc private dynamic var statusValue: Int = TransactionStatus.pending.rawValue
...
public class LocalTxPool: NSObject {
    ...
    public func getTransactions(wallet: ViewWalletInterface) -> [HistoryTxModel] {
        return (try! Realm()).objects(LocalTxModel.self).filter({
            $0.from.uppercased() == wallet.address.uppercased() && $0.assetID.uppercased() == wallet.assetID.uppercased()
        }).map({
...
extension LocalTxModel {
    public func getTx(asset: AssetInterface) -> HistoryTxModel {
        var tx = HistoryTxModel(chainType: asset.chainType,
                                asset: asset,
                                txhash: txHash,
                                toAddress: to,
                                fromAddress: from,
