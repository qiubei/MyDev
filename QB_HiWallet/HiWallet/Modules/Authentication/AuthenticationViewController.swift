...
extension AuthenticationViewController {
    ...
    func loadAccount() {
        ...
        userStore.update { user in
            ...
            App.isLogined = true
            //推送上传
            PushManager.shared.uploadDevice()
            UserManager.shared.register {}
            AuthenticationService.shared.closeAuthentication()
...
//                        }
                    ...
                    //推送上传 + 上报默认创建的钱包
                    PushManager.shared.uploadDevice()
                    let allwallets = (inject() as WalletInteractorInterface).getAllWallet()
                    PushManager.shared.uploadWallets(assets: allwallets)

                    LoginService.shared.close()
