...
extension WalletSettingController {
    ...
    private func clearData() {
        ...
        userStorage.remove(userID: viewUser.id)
        viewUserService.remove(viewUser)

        PushManager.shared.removeUser()
        PushManager.shared.deleteAllMessage()
        NotificationName.changeNotiifcationInfo.emit()
        NotificationName.deleteAllNotification.emit()
