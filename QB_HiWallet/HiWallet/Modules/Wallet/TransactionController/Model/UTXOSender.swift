...
class UTXOSender: TranstonInterface {
    var gasLimit: Int = 0
    ...
    func loadFeeData(callback: @escaping (Bool) -> Void) {
        if isBtc {
            ...
            let feeModel = BtcFeeList.defautFee()
            btcfee = feeModel
            callback(true)
        }
    }
    ...
    func validInput() -> ValidInputMessageType {
        ...
        if let amountValue = Double(amount) {
            ...
            guard getdigitNumber(value: amount) <= 8 else {
                return ValidInputMessageType.toast("小数点不能超过 8 位！".localized())
            }
    ...
    init(walletInfo: ViewWalletInterface) {
        ...
        let cryptoWallet = CryptoWallet(
            ...
            etherScanApiKey: "")
        utxoCoin = walletInfo.asset as? ChainType ?? ChainType.bitcoin
    ...
    func getPopData() -> [SendPopViewModel] {
        ...
        // 发送人
        let fromModel = SendPopViewModel(title: "发送账户".localized(), topDesc: wallet.name, bottomDesc: wallet.address)
        // 接受人
        let toModel = SendPopViewModel(title: "发送到".localized(), topDesc: contact?.note ?? "", bottomDesc: address)
        // 速度
        let fastSpeed = NormalPopModel(speedDesc: isBtc ? "20分钟内".localized() : "加速".localized(),
                                       Fee: getBtcFeeDesc(sat: btcfee!.fastestFee),
                                       image: UIImage.init(named: "icon_tx_speed_fast"))
        let fastSpeed2 = NormalPopModel(speedDesc: isBtc ? "30分钟内".localized() : "正常".localized(),
                                        Fee: getBtcFeeDesc(sat: btcfee!.halfHourFee),
                                        image: UIImage.init(named: "icon_tx_speed_normal"))
        let fastSpeed3 = NormalPopModel(speedDesc: isBtc ? "1小时内".localized() : "一般".localized(),
                                        Fee: getBtcFeeDesc(sat: btcfee!.hourFee),
                                        image: UIImage.init(named: "icon_tx_speed_slow"))

        feeList = [fastSpeed, fastSpeed2, fastSpeed3]
        feePrice = [btcfee!.fastestFee, btcfee!.halfHourFee, btcfee!.hourFee]
        // 默认选中中间
        // 时间
        let processingModel = SendPopViewModel(title: isBtc ? "到账时间".localized() : "手续费".localized(), topDesc: isBtc ? fastSpeed2.speedDesc : "", bottomDesc: fastSpeed2.Fee, nextList: feeList)

        let totalModel = SendPopViewModel(title: "总计".localized(), topDesc: "\(getTotalAmount())" + " \(wallet.symbol)", bottomDesc: BalanceFormatter.getCurreyPrice(fullName: wallet.fullName, value: getTotalAmount().doubleValue))
    ...
    func reloadPopDataWithIndex(index: Int) -> [SendPopViewModel] {
        ...
        // 更新选择
        let processingModel = SendPopViewModel(title: isBtc ? "到账时间".localized() : "手续费".localized(), topDesc: speed.speedDesc, bottomDesc: speed.Fee, nextList: feeList)
        ...
        do {
            ...
            utxoService!.sendTransaction(with: rawTx) { [weak self] in
                ...
                case let .success(object):
                    ...
                    //ltc
                    if object.txid.contains("404") || object.txid.contains("{") {
                        callback((false, ""))
                    } else {
...
extension UTXOSender {
    ...
    func getBtcFeeDesc(sat: String) -> String {
        return "\(getBtcFee(sat: sat))" + " \(wallet.symbol)" + "≈ \(BalanceFormatter.getCurreyPrice(fullName: wallet.fullName, value: getBtcFee(sat: sat).doubleValue))"
    }
    ...
    func createRawTx() throws -> String {
        ...
        let address = try LegacyAddress(self.address, coin: wrapCoin(coin: utxoCoin))
