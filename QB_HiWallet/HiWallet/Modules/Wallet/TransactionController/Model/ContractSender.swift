...
class ContractSender {
    ...
    func getPopData() -> [SendPopViewModel] {
        ...
        // 发送人
        let fromModel = SendPopViewModel(title: "发送账户".localized(), topDesc: "", bottomDesc: wallet.address)
        // 接受人
        ...
        let fastSpeed = NormalPopModel(speedDesc: String(format: "预计 %@ 分钟内".localized(), ethFee!.fastWait),
                                       Fee: getAmountWithSymble(value: getFreeWithGas(gasPrice: ethFee!.fastest)) + BalanceFormatter.getCurreyPrice(fullName: ChainType.ethereum.fullName,value: getFreeWithGas(gasPrice: ethFee!.fastest)),
        ...
        let fastSpeed2 = NormalPopModel(speedDesc: String(format: "预计 %@ 分钟内".localized(), ethFee!.avgWait),
                                        Fee: getAmountWithSymble(value: getFreeWithGas(gasPrice: ethFee!.average)) + BalanceFormatter.getCurreyPrice(fullName: ChainType.ethereum.fullName,value: getFreeWithGas(gasPrice: ethFee!.average)),
        ...
        let fastSpeed3 = NormalPopModel(speedDesc: String(format: "预计 %@ 分钟内".localized(), ethFee!.safeLowWait),
                                        Fee: getAmountWithSymble(value: getFreeWithGas(gasPrice: ethFee!.safeLow)) + BalanceFormatter.getCurreyPrice(fullName: ChainType.ethereum.fullName,value: getFreeWithGas(gasPrice: ethFee!.safeLow)),
        ...
        let totalModel = SendPopViewModel(title: "总计".localized(), topDesc: "\(getTotalAmount())" + "ETH", bottomDesc: BalanceFormatter.getCurreyPrice(fullName: ChainType.ethereum.fullName,value: getTotalAmount().doubleValue))
    ...
    func reloadPopDataWithIndex(index: Int) -> [SendPopViewModel] {
        ...
        gasPrice = Int(Double(feePrice[index])! / pow(10, 9))

        return dataList
    }
}

extension ContractSender {
