...
class WalletMainViewController: BaseViewController, UINavigationControllerDelegate {
    ...
    @IBOutlet weak var notificationRightConstrain: NSLayoutConstraint!
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        addEvents()
        notificationView.numberView.number = viewModel.notificationUnreadNumber
        
        if viewModel.notificationUnreadNumber > 0 {
            notificationRightConstrain.constant = 15
        } else {
            notificationRightConstrain.constant = 0
        }
    ...
    override func setup() {
        ...
        tableview.contentInset = UIEdgeInsets(top: statusHeight - 14, left: 0, bottom: 0, right: 0)
...
//        navigationController?.pushViewController(controller, animated: true)
    ...
    private func addEvents() {
        ...
        notificationView.bk_(whenTapped: { [weak self] in
    ...
    private func notificationInfoChangeAction(_ notification: Notification) {
        let number = viewModel.notificationUnreadNumber
        notificationView.numberView.number = number
        if number > 0 {
            notificationRightConstrain.constant = 15
        } else {
            notificationRightConstrain.constant = 0
...
extension WalletMainViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let controller = AssetCardViewController()
