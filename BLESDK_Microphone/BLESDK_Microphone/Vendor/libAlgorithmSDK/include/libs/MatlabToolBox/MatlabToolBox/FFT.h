...
#ifndef MatlabToolBox_MatlabToolBox_FFT_H
#define MatlabToolBox_MatlabToolBox_FFT_H
 ...
 *  @return 输出数据（复数）
 */
template <typename T> std::vector<std::complex<T>> dft(const std::vector<std::complex<T>> &data, size_t N);
 ...
 * @return 输出数据（复数）
 */
template <typename T> std::vector<std::complex<T>> idft(const std::vector<std::complex<T>> &data, size_t N);
...
template <typename T> std::vector<std::complex<T>> fft(const std::vector<std::complex<T>> &data) {
    ...
    if (BasicInner::nextPowerOf2(data.size()) != data.size())
...
template <typename T> std::vector<std::complex<T>> fft(const std::vector<std::complex<T>> &data, size_t N) {
    ...
    if (BasicInner::nextPowerOf2(N) != N) {
...
template <typename T> std::vector<std::complex<T>> raderFFT(const std::vector<std::complex<T>> &data) {
    ...
    auto exp = MatlabToolBox::expModuloN(g, static_cast<size_t>(1), N);
    auto expInverse = MatlabToolBox::expModuloNInverse(g, static_cast<size_t>(1), N);
    ...
    // 补零
    auto M = BasicInner::nextPowerOf2(2*N-3);
    if (M != N-1) {
