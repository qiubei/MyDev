...
#ifndef MatlabToolBox_MatlabToolBox_Wavelet_H
#define MatlabToolBox_MatlabToolBox_Wavelet_H
 ...
 */
template <typename T> std::pair<std::vector<T>, std::vector<size_t>> wavedec(const std::vector<T> &x, size_t n, const std::string &wname);
 ...
 *  @return     返回结果
 */
template <typename T> std::vector<T> waverec(const std::vector<T> &c, const std::vector<size_t> &l, const std::string &wname);
...
namespace EnterTech{
namespace MatlabToolBox{
template <typename T> std::vector<T> waverec(const std::vector<T> &c, const std::vector<size_t> &l,
...
template <typename T> std::pair<std::vector<T>, std::vector<size_t>> wavedec(const std::vector<T> &x,
                                                                             size_t n, const std::string &wname){
    static_assert(std::is_floating_point<T>::value, "T must float type!");
...
template <typename T> std::vector<T> detcoef(const std::vector<T> &coefs, const std::vector<size_t> &longs, size_t levels) {
    ...
    for (auto&& item : tmp)
        ++item;
    auto first = truncate(tmp, tmp.size() - 2, -1, 1);

    auto longsM = truncate(longs, longs.size() - 1, -1, 2);
    ...
    auto k = levels;
    return truncate(coefs, first[k - 1], last[k - 1]);
}
...
template <typename T> std::vector<T> wextend(const std::string &type, const std::string &mode,
    ...
    if((type=="1D" || type=="1d") && mode == "sym"){
        ...
        if (lx < lf) {
            ...
            auto J = MatlabToolBox::find<int>(I, [&](int input){ return input > lx;});
            while(MatlabToolBox::any(J)) {
                for (auto index : J)
                ...
                J = MatlabToolBox::find<int>(I, [&](int input){ return input > lx;});
            }
        }
