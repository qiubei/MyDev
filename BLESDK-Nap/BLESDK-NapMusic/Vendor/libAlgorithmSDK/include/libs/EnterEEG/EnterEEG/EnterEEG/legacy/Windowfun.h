//
// Created by 雷京颢 on 2018/6/12.
//

#ifndef ENTEREEG_LIBS_WINDOWFUN_H
#define ENTEREEG_LIBS_WINDOWFUN_H
...
template <typename T> std::vector<T> calc_window(size_t m, size_t n, const std::string& window) {
    ...
    for (unsigned i = 0; i <= m - 1; ++i) {
        w.emplace_back(0.54 - 0.46 * cos(2 * M_PI * i / (n - 1)));
    }
    return w;
}
...
#endif //ENTEREEG_LIBS_WINDOWFUN_H
